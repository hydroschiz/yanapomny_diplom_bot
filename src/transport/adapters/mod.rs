...
#[async_trait::async_trait]
impl BotTransport for teloxide::Bot {
    async fn send_text(&self, peer_id: i64, text: &str) -> anyhow::Result<()> {
        use teloxide::prelude::*;

        self.send_message(ChatId(peer_id), text).await?;
        Ok(())
    }
    ...
    async fn send_with_keyboard(
        ...
        let markup: ReplyMarkup = keyboard.clone().into();
        self.send_message(ChatId(peer_id), text)
            .reply_markup(markup)
            .await?;
        Ok(())
    }
    ...
    async fn answer_callback(
        ...
        let request =
            self.answer_callback_query(teloxide::types::CallbackQueryId(event_id.to_string()));
